...
async def stream_chat(payload: ChatRequest, service: ChatService = Depends(get_chat_service)):
    async def events():
        pending = None
        async with aclosing(service.stream(payload.messages, payload.use_tools, payload.model, payload.think, payload.image)) as stream:
            try:
...
async def chat(
    ...
    try:
        return await service.run(payload.messages, payload.use_tools, payload.model, payload.think, payload.image)
