...
export function renderMessageContent(message, text, structuredSources) {
  ...
  sources.className = 'source-card';
  const summary = document.createElement('summary');
  summary.textContent = `검색 근거 ${sourceLines.length}개`;
  const list = document.createElement('ol');
  for (const line of sourceLines) {
    const item = document.createElement('li');
    // 번호는 ol이 표시하므로 서버 문자열의 선행 [n]은 화면에서만 제거한다.
    item.textContent = line.replace(/^\[\d+\]\s*/, '');
    list.appendChild(item);
  }
  sources.append(summary, list);
  message.bubble.insertBefore(sources, message.meta);
...
export function addMessage(text, role) {
  ...
  const message = { article, bubble, body, meta, sources: null };
